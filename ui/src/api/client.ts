    ...
    try {
      ...
      return {
        items: response.map((f: any) => ({
          ...
          match: f.suggestedSeriesId ? {
            seriesId: f.suggestedSeriesId,
            seriesTitle: f.suggestedSeriesTitle ?? `Series #${f.suggestedSeriesId}`,
            issueId: null,
            editionId: f.suggestedEditionId ?? null,
            confidence: f.parseConfidence ?? 0,
          } : null,
          status: f.suggestedSeriesId ? 'matched' : 'unmatched',
        })),
        page: 1,
        pageSize: 100,
