...
public class PullListService : IPullListService
{
    ...
    /// <inheritdoc />
    public void PrefetchAdjacentWeeksAsync(DateTime currentWeekOf, bool prefetchDiscovery = true, bool prefetchPullList = true)
    {
        // Fire-and-forget: Prefetch adjacent weeks in background to warm the cache
        // This improves perceived performance when navigating weeks
        _ = Task.Run(async () =>
        {
            try
            {
                var (currentWeekStart, _) = GetWeekBoundaries(currentWeekOf);
                ...
                if (prefetchPullList)
                {
                    // Prefetch next week's pull list
                    var nextPullListCacheKey = _cacheService.GenerateKey(CacheKeys.PullListWeek, nextWeek.Date);
                    if (!_cacheService.Exists(nextPullListCacheKey))
                    {
                        _logger.LogDebug("Prefetching pull list for week of {Date}", nextWeek.Date);
                        tasks.Add(GetWeeklyReleasesAsync(nextWeek));
                    }

                    // Prefetch previous week's pull list
                    var prevPullListCacheKey = _cacheService.GenerateKey(CacheKeys.PullListWeek, prevWeek.Date);
                    if (!_cacheService.Exists(prevPullListCacheKey))
                    {
                        _logger.LogDebug("Prefetching pull list for week of {Date}", prevWeek.Date);
                        tasks.Add(GetWeeklyReleasesAsync(prevWeek));
                    }
                }

                if (prefetchDiscovery)
                {
                    // Prefetch next week's discovery (ComicVine data)
                    var nextDiscoveryCacheKey = _cacheService.GenerateKey(CacheKeys.PullListDiscovery, nextWeek.Date);
                    if (!_cacheService.Exists(nextDiscoveryCacheKey))
                    {
                        _logger.LogDebug("Prefetching discovery for week of {Date}", nextWeek.Date);
                        tasks.Add(GetWeeklyDiscoveryAsync(nextWeek));
                    }

                    // Prefetch previous week's discovery
                    var prevDiscoveryCacheKey = _cacheService.GenerateKey(CacheKeys.PullListDiscovery, prevWeek.Date);
                    if (!_cacheService.Exists(prevDiscoveryCacheKey))
                    {
                        _logger.LogDebug("Prefetching discovery for week of {Date}", prevWeek.Date);
                        tasks.Add(GetWeeklyDiscoveryAsync(prevWeek));
                    }
                }
                ...
                if (tasks.Count > 0)
                {
                    ...
                    _logger.LogDebug("Prefetched {Count} adjacent week caches", tasks.Count);
                }
            }
            catch (Exception ex)
            {
                // Don't throw - prefetching is best-effort and should not affect the main request
                _logger.LogWarning(ex, "Error during adjacent week prefetching");
            }
        });
    }
