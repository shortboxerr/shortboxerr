...
**Git:** Integrate via **one pull request to `main`** (not direct push). **Batch** code + `docs/WORKLOG.md` / `docs/BACKLOG.md` / `docs/SELF_CHECK.md` on the topic branch **before** opening the PR. Steps: `docs/CONTRIBUTING.md` → *Typical workflow (merge to `main`)*. Merge audit: `docs/AUTO_MERGE_LOG.md` (updated by Actions on merge).
